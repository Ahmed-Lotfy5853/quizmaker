      ...
      body: Container(
        ...
        child: Padding(
          padding: EdgeInsets.only(
              ...
              top: height / 10.3,
              bottom: height / 16.5),
          child: buildBlocWidget('AuthInitial',height, width, bigFontsize, smallFontsize),
        ),
      ),
    );
  }
...
//))  ;
  ...
  buildBlocWidget(
      String state,
      double height, double width, double bigFontsize, double smallFontsize) {
      if (state == 'AuthInitial') {
      if (isLogin) {
        return buildLoginForm(height, width, bigFontsize, smallFontsize);
      } else {
        return buildRegisteringForm(
            height, width, bigFontsize, smallFontsize);
      }
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
        Navigator.pushNamed(
            context, current_user!.isTeacher ?  bottomNavStudentScreen : teacherNavBar);
      });
      return Container();
    } else {
  ...
  Future<void> getProfile(String uId, BuildContext context) async {
    FirebaseFirestore.instance.collection(studentsCollection).doc(uId).get().then(
        ...
        if (data != null) {
          ...
          Navigator.push(context,MaterialPageRoute(builder: (context)=>BottomNavBarStudentScreen()) );
        } else {
          FirebaseFirestore.instance.collection(teachersCollection).doc(uId).get().then(
                (DocumentSnapshot doc) {
